	...
	1	 LOWER(COLUMN) = trasforma in minuscolo i dati stringa della colonna selezionata
	2	 UPPER(COLUMN) = trasforma in maiuscolo i dati stringa della colonna selezionata
	3	 TRIM(COLUMN) = rimuovo ciò che viene indicato tra parentesi:
	...
	4	 LENGTH(COLUMN) = ritorna la lunghezza dei dati inseriti, ovvero da quanti caratteri è composto il dato inserito
	5	 SUBSTRING(COLUMN, START, LENGTH) = recupera il numero di caratteri specificato in LENGHT partendo dallo START
	6	 ROUND(COLUMN, DECIMAL_PLACES) = server per arrotondare I numeri decimali e con il numero che metto al posto di DECIMAL_PLACES specifico il numero di cifre decimali a cui voglio arrotondare
	7	 DATE_ADD(COLUMN, INTERVAL EXPRESSION UNIT) = aggiunge giorni/mesi/anni alla data selezionata:
	                                                  - EXPRESSION è il numero che voglio venga aggiunto
	                                                  - UNIT devo specificare DAY/MONTH/YEAR
	8	 DATE_SUB(COLUMN, INTERVAL EXPRESSION UNIT)= sottrae giorni/mesi/anni alla data selezionata
	9	 FORMAT(COLUMN, FORMAT_STRING) = permette di formattare la data nel formato della nazione specificata
	    (se intendeva DATE_FORMAT(COLUMN, FORMAT_STRING) allora serve a formattare la data nel formato indicato in FORMAT_STRING)
	10	Using DATE_ADD or DATE_SUB and the following SQL SELECT NOW();
